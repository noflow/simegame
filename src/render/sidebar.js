...
export function renderSidebar(){
  ...
  if (kc) {
    const introduced = chars.filter(c => (state.relationships && state.relationships[c.id] && state.relationships[c.id].introduced));
...
export function renderMoney(){
  ...
  if (m) m.textContent = `$${window.GameState.state.money}`;
}
