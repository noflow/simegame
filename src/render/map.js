...
export function renderLocation(){
  ...
  // If no world loaded yet, keep the placeholder text shown in index.html
  if (!WORLD || !WORLD.passages) {
    return;
  }

  const passage = WORLD.passages[state.location];
  const base = (typeof passage === 'string') ? passage : ((passage && passage.text) || '');
  ...
  // expand {npc:id} tokens safely even if characters aren't loaded yet
  let html = base.replace(/\{npc:([a-z0-9_\-]+)\}/gi, (m,id)=>{
    const chars = window.GameData?.CHARACTERS?.characters || [];
    const npc = chars.find(c=>c.id===id);
    if (!npc) return '';
    const present = window.GameLogic?.npcHere ? window.GameLogic.npcHere(npc) : false;
    const status = present
      ? `<span class="pill" style="border-color:#2f4732;color:#a6e0a3">here</span>`
      : `<span class="pill" style="border-color:#47322f;color:#e0b0a6">away</span>`;
  ...
  // link syntax [[Label->Destination]]
  html = html.replace(/\[\[(.+?)->(.+?)\]\]/g, (m,label,dest)=> `<a href="#" data-goto="${dest}">${label}</a>`);

  el.innerHTML = html;

  // wire up travel links
  const links = el.querySelectorAll('a[data-goto]');
  ...
  if (loc) loc.textContent = state.location;
}

export function goTo(dest){
  if (!WORLD || !WORLD.passages || !WORLD.passages[dest]) {
    alert(`No such place: ${dest}`);
    return;
  }
  const pass = WORLD.passages[dest];
  if (typeof pass === 'object' && pass && pass.locked) {
    alert(`${dest} is locked right now.`);
    return;
  }
  state.location = dest;
  state.threadVisible = [];
